...
define i1 @pair_overlapped(i32 %fs, i32 %fe, i32 %ss, i32 %se) {
  ...
  %fe_gte_se = icmp sge i32 %fe, %se
  %ss_lte_fs = icmp sle i32 %ss, %fs
  %se_gte_fe = icmp sge i32 %se, %fe
  %f_over_s = and i1 %fs_lte_ss, %fe_gte_se
  %s_over_f = and i1 %ss_lte_fs, %se_gte_fe
  %overlapped = or i1 %f_over_s, %s_over_f
  ret i1 %overlapped
}
...
print_output:
  call i32 (i8*, ...) @printf(i8* getelementptr ([4 x i8], [4 x i8]* @.print_output_format, i64 0, i64 0), i32 %num_overlapped)
  br label %cleanup
cleanup:
